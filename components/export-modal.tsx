  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const dataUrl = await toJpeg(exportRef.current, {
        cacheBust: true, width: 1080, height: 1920, quality: 0.98,
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[1000] flex flex-col items-center justify-center p-6 sm:p-10">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => !isSaving && onClose()} className="absolute inset-0 bg-[#0D0B09]/95 backdrop-blur-3xl" />

          <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative w-[min(280px,65vw)] aspect-[9/16] rounded-[24px] overflow-hidden shadow-[0_32px_96px_-16px_rgba(0,0,0,0.8)] ring-1 ring-white/10" >
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="flex flex-col items-center gap-8 mt-12 relative z-20 w-full px-6" >
            ...
            <button disabled={isSaving} onClick={handleDownload} className={cn( "group flex items-center justify-center gap-3 px-10 py-5 font-italiana text-lg tracking-[0.1em] rounded-full transition-all shadow-xl w-full max-w-[320px]", isSaved ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-[#f2ece0] text-black hover:scale-105 active:scale-95" )} >
              {isSaving ? <HugeiconsIcon icon={Loading03Icon} size={20} className="animate-spin" /> : isSaved ? <HugeiconsIcon icon={CheckmarkCircle02Icon} size={20} /> : <HugeiconsIcon icon={Download01Icon} size={20} className="transition-transform group-hover:translate-y-0.5" />}
              <span>{isSaving ? "generating..." : isSaved ? "success!" : "save to phone"}</span>
            </button>
            <button disabled={isSaving} onClick={onClose} className="flex items-center gap-2 font-jost text-[12px] uppercase tracking-[0.2em] opacity-30 hover:opacity-100 transition-all py-2 disabled:opacity-0">
