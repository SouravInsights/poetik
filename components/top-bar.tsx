...
export function TopBar({ visible, onClear, align, onAlignToggle, tone }: TopBarProps) {
  ...
  return (
    <div
      className={cn(
        "fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-8 pt-[max(env(safe-area-inset-top),20px)] pb-10 transition-all duration-400 ease-in-out",
    ...
    >
      <div className="flex items-center gap-8">
        <Logo onDark={isDark} />
        <div className={cn("h-4 w-[1px] opacity-10", isDark ? "bg-white" : "bg-black")} />
        <button
          onClick={() => {
            trigger("selection");
            onAlignToggle();
          ...
          className={cn(
            "flex items-center gap-2 font-jost text-[10px] font-bold tracking-[0.2em] uppercase transition-all py-2 px-5 rounded-full backdrop-blur-xl border select-none shadow-md",
            isDark 
        ...
        >
          <HugeiconsIcon 
            ...
            strokeWidth={2.5}
          />
          <span className="mt-0.5">{align}</span>
        </button>
      </div>
      <button
        onClick={() => {
      ...
      >
        <HugeiconsIcon icon={Delete02Icon} size={13} strokeWidth={2.5} />
